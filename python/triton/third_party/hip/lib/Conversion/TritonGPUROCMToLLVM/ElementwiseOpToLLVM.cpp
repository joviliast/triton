...
static SmallVector<Value> reorderValues(const SmallVector<Value> &values,
  ...
  if (!inTensorTy || !ouTensorTy)
    return values;
  auto inEncoding =
      dyn_cast<triton::gpu_rocm::DotOperandEncodingAttr>(inTensorTy.getEncoding());
  auto ouEncoding =
      dyn_cast<triton::gpu_rocm::DotOperandEncodingAttr>(ouTensorTy.getEncoding());
  ...
  // reorder elements
  auto parentEncoding =
      dyn_cast<triton::gpu_rocm::MmaEncodingAttr>(ouEncoding.getParent());
  if (!parentEncoding)
    return values;
...
struct CmpIOpConversion
    : public ElementwiseOpConversionBase<triton::gpu_rocm::CmpIOp,
                                         CmpIOpConversion> {
  using Base =
      ElementwiseOpConversionBase<triton::gpu_rocm::CmpIOp, CmpIOpConversion>;
  using Base::Base;
  using Adaptor = typename Base::OpAdaptor;

  // An interface to support variant DestOp builder.
  SmallVector<LLVM::ICmpOp>
  createDestOps(triton::gpu_rocm::CmpIOp op, OpAdaptor adaptor,
...
struct CmpFOpConversion
    : public ElementwiseOpConversionBase<triton::gpu_rocm::CmpFOp,
                                         CmpFOpConversion> {
  using Base =
      ElementwiseOpConversionBase<triton::gpu_rocm::CmpFOp, CmpFOpConversion>;
  using Base::Base;
  using Adaptor = typename Base::OpAdaptor;

  // An interface to support variant DestOp builder.
  static SmallVector<LLVM::FCmpOp>
  createDestOps(triton::gpu_rocm::CmpFOp op, OpAdaptor adaptor,
                ConversionPatternRewriter &rewriter, Type elemTy,
...
#define POPULATE_TERNARY_OP(SRC_OP, DST_OP)                                    \
  ...
  POPULATE_TERNARY_OP(triton::gpu_rocm::SelectOp, LLVM::SelectOp)
