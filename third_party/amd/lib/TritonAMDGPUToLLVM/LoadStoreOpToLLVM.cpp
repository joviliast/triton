    ...
    for (size_t i = 0; i < elemsPerThread; i += vec) {
      ...
      if (tryCollectPartSumInLDS) {
        Value ldsPtr =
            getSharedMemoryBase(loc, rewriter, targetInfo, op.getOperation());
        retVal =
            atomicForReducedGroupsInLds(rewriter, rmwPtr, operand, rmwMask,
                                        ldsPtr, *maybeKind, atomicMemOrdering);
      } else {
...
private:
  Value atomicForReducedGroupsInLds(PatternRewriter &rewriter, Value rmwPtr,
                                    Value operand, Value rmwMask, Value ldsPtr,
    ...
    Value allBits = i64_val(~uint64_t(0));
    Value isDisabled = icmp_eq(rmwMask, false_val());
    rmwPtr = ptrtoint(i64_ty, rmwPtr);
    // set all bits for disabled threads as address
    rmwPtr = or_(rmwPtr, mul(allBits, zext(i64_ty, isDisabled)));
    ...
    auto *mainAlgBlock = rewriter.createBlock(
        curBlock->getParent(), std::next(Region::iterator(curBlock)));
    // Skip everithing if thread is disabled
    rewriter.setInsertionPointToEnd(curBlock);
    Value isEnabled = icmp_ne(rmwPtr, allBits);
    Value defaultRes = undef(operandElemType);
    rewriter.create<LLVM::CondBrOp>(loc, isEnabled, mainAlgBlock, endBlock,
                                    defaultRes);
    rewriter.setInsertionPointToEnd(mainAlgBlock);
    ...
    auto *partialSharedBlock = rewriter.createBlock(
        mainAlgBlock->getParent(), std::next(Region::iterator(mainAlgBlock)));
