...
llvm::SmallVector<Value> computeOffsetsAType(
    ...
    computeTensorElemMappingT fn, const ArrayRef<int64_t> &elemsPerInstr,
    Value warpId, Value laneId, int warpsPerGroup, int numOfElems,
  ...
  auto mapping = fn(rewriter, loc, elemsPerInstr, warpId, laneId, warpsPerGroup,
                    numOfElems, reps, offsets, vectorSize, nonKDim, kDim);
...
llvm::SmallVector<Value> computeOffsetsBType(
  ...
  auto mapping =
      fn(rewriter, loc, tElemsPerInstr, warpId, laneId, warpsPerGroup,
         numOfElems, tReps, toffsets, vectorSize, nonKDim, kDim);
